...
export const assignScoreToReport = async (req, res) => {
  try {
    ...
    try {
      ...
      emitToUser(studentId, "report_graded", {
        score: scoreValue,
        reportTitle: populatedScore.report.title,
        feedback: "A score has been assigned to your report."
      });
    } catch (socketErr) {
    ...
    try {
      const { updateStatsOnGrading, checkAndAwardBadges } = await import("../../../utils/badgeSystem.js");
      await updateStatsOnGrading(studentId, scoreValue);
      await checkAndAwardBadges(studentId, 'report_graded');
    } catch (badgeErr) {
